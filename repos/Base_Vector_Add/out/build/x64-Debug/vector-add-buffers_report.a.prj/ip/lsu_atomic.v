...
parameter SIGNED_OP=0;          // whether the atomic operation is signed or unsigned
...
// +1 for 'atomic' flag, +1 for 'signed' flag
localparam UNUSED_WRITEDATA_WIDTH = WRITEDATAWIDTH - (2 * WIDTH + ATOMIC_WIDTH + BYTE_SELECT_BITS + 1 + 1);
...
begin
  ...
  // avm_writedata contains {valid atomic bit, writedata, cmpdata, atomic_op, signed}
  ...
  assign avm_writedata[2*WIDTH+ATOMIC_WIDTH+BYTE_SELECT_BITS+1:2*WIDTH+ATOMIC_WIDTH+BYTE_SELECT_BITS+1] = SIGNED_OP;
  assign avm_writedata[WRITEDATAWIDTH-1:2*WIDTH+ATOMIC_WIDTH+BYTE_SELECT_BITS+1+1] = { UNUSED_WRITEDATA_WIDTH{1'b0} };
